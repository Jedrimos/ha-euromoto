...
class TestPdfUrl:
    ...
    def test_matches_confirmed_site_layout(self):
        """Regression test: confirmed against the site's own file browser
        (results.bike-promotion.com/#Results/Championship scores/{year}/01 EURO
        MOTO/) - a single cumulative PDF per class directly under "01 EURO MOTO",
        named "Punktestaende EURO MOTO_<CLASS>.pdf" (uppercase, "EURO MOTO" not
        "IDM"), with NO round-numbered subfolder at all."""
        url = _pdf_url("Superbike", 2026)
        assert "01%20EURO%20MOTO" in url
        assert "EURO%20MOTO_SUPERBIKE" in url
        assert "IDM" not in url
...
class TestFetchStandings:
    ...
    @pytest.mark.asyncio
    async def test_fetches_the_single_confirmed_url(self):
        """Regression test: standings live at one fixed, continuously-updated
        URL per class/year (confirmed via the site's own file browser) - not
        a per-round folder. fetch_standings must hit that exact URL and
        return the parsed rows from it directly, with no round guessing."""
        from custom_components.euromoto.pdf_parser import EuroMotoPdfParser
        ...
        mock_resp.__aexit__ = AsyncMock(return_value=False)
        mock_resp.raise_for_status = MagicMock()
        mock_resp.read = AsyncMock(return_value=b"fake-pdf-bytes")
        mock_session = MagicMock()
        mock_session.get = MagicMock(return_value=mock_resp)
        ...
        assert "01%20EURO%20MOTO" in called_url
        assert "EURO%20MOTO_SUPERBIKE" in called_url
